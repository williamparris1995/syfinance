...
/// SecurityType → 御财金色板(对齐 A-od TYPE_META colors)。
...
class HoldingPieChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
            ...
            children: [
              PieChart(
                PieChartData(
                  ...
                  sections: _sections(ordered, total),
                ),
              ),
        ...
        Expanded(child: _legend(ordered, total)),
      ],
    );
  }

  List<PieChartSectionData> _sections(
      List<HoldingSlice> ordered, int total) {
    if (total <= 0) {
      // 空数据占位:单一灰色满环。
      return [
        PieChartSectionData(
    ...
    return [
      for (final sl in ordered)
        PieChartSectionData(
          ...
          color: kHoldingTypeColors[sl.type] ?? const Color(0xFFB9B2A6),
          radius: 22,
          showTitle: false,
        ),
    ];
  ...
  Widget _legend(List<HoldingSlice> ordered, int total) {
      ...
      children: [
        for (final t in order)
          Padding(
            ...
            child: _LegendRow(
              color: kHoldingTypeColors[t] ?? const Color(0xFFB9B2A6),
...
class _LegendRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final labelColor = dim ? AppColors.muted : AppColors.fg;
    final valColor = dim ? AppColors.muted : AppColors.fg;
    return Row(
      children: [
        Container(
          width: 9,
