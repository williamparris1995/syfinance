...
type mockDebtSyncer struct {
	calls     atomic.Int64
	perTenant int                 // returned count per tenant
	errOn     map[uuid.UUID]error // simulate per-tenant error
}

func (m *mockDebtSyncer) SyncAllDebts(_ context.Context, tenantID uuid.UUID) (int, error) {
	m.calls.Add(1)
	if m.errOn != nil {
...
func TestStartRunsOnceImmediately(t *testing.T) {
	...
	syncer := &mockDebtSyncer{perTenant: 2}
	...
	if !waitForCalls(syncer, 2, 50*time.Millisecond) {
		t.Fatalf("SyncAllDebts not fanned out within 50ms; calls=%d", syncer.calls.Load())
	}
}
...
func TestStartDoesNotSyncBeforeInterval(t *testing.T) {
	...
	syncer := &mockDebtSyncer{perTenant: 1}
...
func TestSyncNowContinuesPastTenantError(t *testing.T) {
	...
	syncer := &mockDebtSyncer{
		perTenant: 3,
		errOn:     map[uuid.UUID]error{bad: errors.New("snapshot repo unavailable")},
	}
...
func waitForCalls(syncer *mockDebtSyncer, want int64, max time.Duration) bool {
