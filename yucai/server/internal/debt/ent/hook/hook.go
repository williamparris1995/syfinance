...
// The DebtDetailsFunc type is an adapter to allow the use of ordinary
// function as DebtDetails mutator.
type DebtDetailsFunc func(context.Context, *ent.DebtDetailsMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DebtDetailsFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DebtDetailsMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f PaymentScheduleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PaymentScheduleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PaymentScheduleMutation", m)
}
