...
async fn test_seed_data_level1_accounts() {
    ...
    assert!(
        codes.contains(&"1000".to_string()),
        "Should have 资产 (1000)"
    );
    assert!(
        codes.contains(&"2000".to_string()),
        "Should have 负债 (2000)"
    );
    assert!(
        codes.contains(&"3000".to_string()),
        "Should have 权益 (3000)"
    );
    assert!(
        codes.contains(&"4000".to_string()),
        "Should have 收入 (4000)"
    );
    assert!(
        codes.contains(&"5000".to_string()),
        "Should have 支出 (5000)"
    );
}
...
async fn test_seed_data_level2_accounts() {
    ...
    assert!(
        level2_accounts.len() >= 8,
        "Should have at least 8 level 2 accounts"
    );

    let codes: Vec<String> = level2_accounts.iter().map(|a| a.code.clone()).collect();
    assert!(
        codes.contains(&"1001".to_string()),
        "Should have 库存现金 (1001)"
    );
    assert!(
        codes.contains(&"1002".to_string()),
        "Should have 银行存款 (1002)"
    );
    assert!(
        codes.contains(&"1012".to_string()),
        "Should have 其他货币资金 (1012)"
    );
}
...
async fn test_hierarchical_query_get_children() {
    ...
    assert!(
        children.len() >= 3,
        "资产 (1000) should have at least 3 children"
    );

    let codes: Vec<String> = children.iter().map(|a| a.code.clone()).collect();
    assert!(
        codes.contains(&"1001".to_string()),
        "Should include 库存现金 (1001)"
    );
    assert!(
        codes.contains(&"1002".to_string()),
        "Should include 银行存款 (1002)"
    );
    assert!(
        codes.contains(&"1012".to_string()),
        "Should include 其他货币资金 (1012)"
    );
    ...
    for child in &children {
        assert_eq!(
            child.parent_code,
            Some("1000".to_string()),
            "All children should have parent_code = 1000"
        );
...
async fn test_list_by_type() {
    ...
    assert!(
        asset_accounts.len() >= 4,
        "Should have at least 4 asset accounts (1 level 1 + 3 level 2)"
    );
