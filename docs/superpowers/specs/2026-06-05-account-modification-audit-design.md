...
| P3 | P16 | Decimal TEXT 存储 | 数据完整性，无法做数值查询 |
| P1 | P23 | 列表页缺少总资产/总负债/净资产摘要 | 用户核心信息需求 |
| P1 | P24 | 列表页类型扁平排列不匹配三类模型 | 信息架构 |
| P1 | P26/P31/P32 | 删除无确认弹窗无级联说明 | 操作安全性 |
| P2 | P4/P28/P29 | 两个创建入口不一致+预览不全 | 代码维护+UX一致性 |
| P2 | P3/P30 | 编辑Sheet宽度受限+不可变字段无解释 | UX |
| P2 | P8 | 修改初始余额无确认 | 金融安全 |
