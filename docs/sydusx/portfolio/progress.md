...
- Current sprint: R8 sprint-3(F7 ✅ `9eee8de5` + F9 ✅ `95c11fa6`;F8 标签维度待认领)+ R9 sprint-1(**F10 离线写缓冲 ✅ 2026-09-03 merge `39063da2`**;F11 server 同步 RPC → F12 状态/冲突 UI → F13 e2e 待做)。sprint-2 ✅ done(2026-09-03,F6 E2E 全模块关联链路补全,merge `160c3f30`:**模块修改后标准回归 = `make client-e2e`[管道 47 测试] / 出问题定位用 `make client-e2e-ui`[UI 17 测试],均支持 `F=` 单文件;种子→断言→测后删库,真实库零接触**);sprint-1(F1-F4)done;F4-P2(~306 处)与 backlog 见 sprint-2 defer(标签反查/category 接线/生产疑点 6 处等)。
