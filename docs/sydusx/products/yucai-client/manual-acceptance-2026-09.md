...
- [x] A-2 前置发现·托盘右键无菜单(2026-09-16):根因=tray_manager 0.5.3 原生右键只发事件不弹菜单,`popUpContextMenu()` 自 R7 起从未接入——修复 `c5d8639f`,v1.0.2 带出;**升级到 1.0.2 后复验右键**。
- [x] 标题栏与侧栏徽标重复(2026-09-16):拍板=标题栏去徽标去文字只留窗口钮(同 `c5d8639f`)。
- [x] 托盘图标与徽标观感不一(2026-09-16):拍板=**暂缓国际化符号标重设计,先维持「御」字徽标家族**(exe 御全形/托盘印章简化形/侧栏 御 徽标);符号标三变体留 .scratch 备查,重设计记 backlog。
