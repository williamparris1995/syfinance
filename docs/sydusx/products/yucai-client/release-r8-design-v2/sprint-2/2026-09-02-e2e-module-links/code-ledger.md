...
| T5 全量回归门 | ✅ done | 1(golden 卫生:首跑红 → 定位为仓库金图未跟踪地雷,非本分支改动所致) | make client-e2e 10 文件 47 测试 exit 0;F= 单文件绿;analyze 508 基线 0 新增;flutter test 1188 全绿;测试库测后已删。**附带修复**:.gitignore `*.png` 全局规则挡掉 3 张金图(A2 审计金图 + F5d 菜单守卫 ×2)→ 主仓库靠本地文件幸存而绿,任何全新 checkout/worktree 必红;反向规则 `!yucai/client/**/goldens/**/*.png` + 3 金图入库,随 merge 回 main |
