...
- 2026-09-15 **sprint-7 立项**(应用菜单+品牌图标,brainstorm 四项拍板):[sprint-7](sprint-7/sprint.md) — **F22 顶栏应用菜单**(托盘功能应用内平价入口:显示/立即检查/设置/退出,MenuAnchor 复用 F5 yucaiMenuStyle)+ 关闭行为设置化(隐藏到托盘[默认]/退出程序 + 首次一次性提示;对齐 Microsoft Win32 通知区指南双 surface 共识)+ **F23 字符标「御」图标全套**(墨底+鎏金渐变「御」,多尺寸 ICO 16/24/32/48/64/256 替换 runner 图标与托盘图标,不用 flutter_launcher_icons[Windows 单尺寸 #573])。
