...
- 2026-09-02 **sprint-2 立项**(测试基础线,非设计迁移):[sprint-2](sprint-2/sprint.md) — **F6 E2E 全模块关联链路补全**(补齐订阅→自动记账/模板+标签/预算目标联动/债权收回+备份往返 4 组链路断言,`make client-e2e` 成为标准回归入口;基于 90d0f5c2 基础设施,隔离测试库测后即删,手动触发)。F5 menu-anchor 由另一会话进行中(worktree `r8-f5`),不属本 sprint 追踪。
