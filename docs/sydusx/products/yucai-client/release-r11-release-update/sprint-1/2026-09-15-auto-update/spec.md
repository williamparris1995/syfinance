...
- **FR-1 发布流水线**:GitHub Actions workflow(tag `v*` 触发):checkout → flutter build windows --release → ISCC 安装包 → 创建 GitHub Release(附安装包)→ 生成 appcast.xml(条目:版本/安装包 URL/changelog 取 tag 注释)→ EdDSA 签名 → appcast 附加到 Release。零手工分发步骤。
...
- **FR-3 密钥管理**:EdDSA 密钥对一次性生成(脚本入库);**私钥**入 GitHub Actions Secrets(`APPCAST_EDDSA_PRIVATE_KEY`),**公钥**烤入客户端常量(WinSparkle 验签用)。
...
| 校验档位 | EdDSA vs 无 vs Authenticode | EdDSA 清单签名(用户拍板) |
